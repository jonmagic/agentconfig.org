...
function ExpandedRow({ row }: ExpandedRowProps): VNode {
  const [copiedLocation, setCopiedLocation] = useState<'copilot' | 'claude' | 'codex' | null>(null)

  const handleCopy = async (provider: 'copilot' | 'claude' | 'codex', location: string) => {
    try {
      ...
      <td colSpan={4} className="px-4 py-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">{row.codex.implementation}</p>
            <div className="flex items-center gap-2">
              <code className="flex-1 text-xs bg-background px-2 py-1.5 rounded font-mono text-foreground border border-border">
                {row.codex.location}
              </code>
